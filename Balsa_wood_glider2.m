...
W = rho*g*V;
x_ac_HT_bar = x_ac_HT/c;
CMih = (-CL_alpha)*etaHT*(S_H/S)*(x_ac_HT_bar-x_cg_bar);
alpha_trim = (-CM0-CMih*i_t)/(CMalpha);

delta = 0.04; % AR=5 and taper ratio =1
e = (1+delta)^(-1);
CL_alpha_wf = 0.11/(1+57.3*0.11/(pi*e*AR));
CL_alpha_t = 0.11/(1+57.3*0.11/(pi*e*AR));

CLalpha0 = CL_alpha_wf*i_w - CL_alpha_t*i_t*(S_H/S);
CL_trim = CLalpha0 + CL_alpha*alpha_trim;
Vtrim = sqrt(2*W/(CL_trim* rho *S));
